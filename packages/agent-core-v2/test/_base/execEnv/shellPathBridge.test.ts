/**
 * Shell path bridge (vendored `_base/execEnv` module).
 *
 * Mirrors `packages/kaos/test/shell-path-bridge.test.ts` against the vendored
 * copy — same injected `execFileSync` / `isFile` fakes, so the suite runs
