...
describe('useModelProviderState — skill cache refresh', () => {
  ...
  it('does not refresh a failed skill load after it is no longer active', async () => {
    apiMock.listSkills.mockReset();
    apiMock.listSkills.mockRejectedValueOnce(new Error('session unavailable'));
    const provider = useModelProviderState(createState(), {
      pushOperationFailure: vi.fn(),
      refreshSessionStatus: vi.fn(async () => {}),
      persistSessionProfile: vi.fn(async () => true),
    ...
    await provider.loadSkillsForSession('archived-session');
    await provider.refreshLoadedSkills();

    expect(apiMock.listSkills).toHaveBeenCalledTimes(1);
  });

  it('invalidates a session skill cache only when its config refresh fails', async () => {
    apiMock.listSkills.mockReset();
    apiMock.listSkills.mockImplementation((sessionId: string) =>
      sessionId === 'sess_1'
        ? Promise.reject(new Error('session unavailable'))
        : Promise.resolve([{ name: 'fresh-other-skill', description: '', source: 'project' }]),
    );
    const provider = useModelProviderState(createState(), {
      pushOperationFailure: vi.fn(),
      refreshSessionStatus: vi.fn(async () => {}),
    ...
    const stale = [{ name: 'stale-session-skill', description: '', source: 'project' as const }];
    provider.skillsBySession.value = { sess_1: stale, sess_2: [] };

    await provider.loadSkillsForSession('sess_1');
    expect(provider.skillsBySession.value['sess_1']).toEqual(stale);

    await provider.refreshLoadedSkills();
    expect(provider.skillsBySession.value['sess_1']).toBeUndefined();
    expect(provider.skillsBySession.value['sess_2']?.[0]?.name).toBe('fresh-other-skill');
  });

  it('invalidates a workspace skill cache only when its config refresh fails', async () => {
    apiMock.listSkillsForWorkspace.mockReset();
    apiMock.listSkillsForWorkspace.mockImplementation((workspaceId: string) =>
    ...
    const stale = [{ name: 'stale-workspace-skill', description: '', source: 'project' as const }];
    provider.skillsByWorkspace.value = { workspace_1: stale, workspace_2: [] };

    await provider.loadSkillsForWorkspace('workspace_1');
    expect(provider.skillsByWorkspace.value['workspace_1']).toEqual(stale);

    await provider.refreshLoadedSkills();
    expect(provider.skillsByWorkspace.value['workspace_1']).toBeUndefined();
    expect(provider.skillsByWorkspace.value['workspace_2']?.[0]?.name).toBe('fresh-other-skill');
  });
});
