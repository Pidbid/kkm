...
const executableName = process.platform === 'win32' ? 'kkm.exe' : 'kkm';
...
describe('native release artifacts', () => {
  afterEach(() => {
    ...
    rmSync(resolve(artifactsDir, `kkm-${target}.zip`), { force: true });
    rmSync(resolve(artifactsDir, `kkm-${target}.zip.sha256`), { force: true });
  });
  ...
  it('packages the native binary as a zip archive and checksums the archive', async () => {
    ...
    const archivePath = resolve(artifactsDir, `kkm-${target}.zip`);
  ...
  it('produces a manifest from zip archive checksums', async () => {
    ...
    await writeFile(join(releaseDir, 'kkm-darwin-arm64.zip'), archiveBytes);
    await writeFile(
      join(releaseDir, 'kkm-darwin-arm64.zip.sha256'),
      `${checksum}  kkm-darwin-arm64.zip\n`,
    );
