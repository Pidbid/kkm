 ...
 *
 * Input dir must contain files matching: kkm-<target>.zip.sha256
...
const sumFiles = entries.filter((f) => /^kkm-[a-z0-9-]+\.zip\.sha256$/.test(f));

if (sumFiles.length === 0) {
  console.error(`No kkm-<target>.zip.sha256 files found in ${inputDir}`);
  process.exit(1);
}
...
for (const sumFile of sumFiles.sort()) {
  ...
  // kkm-darwin-arm64.zip → darwin-arm64
  const target = filename.replace(/^kkm-/, '').replace(/\.zip$/, '');
